...
void printConfigGodz() {
    ...
    snprintf(buf, sizeof(buf), "D%02d-%02d 1-2+:3-A+\nN%02d-%02d 4-5+:6-B+ \nnight shift%02d 7-8+\nCD h+- 90 m+-", gi_Hour_Day_Start, gi_Hour_Day_End,
            gi_Hour_Night_Start, gi_Hour_Night_End, gi_Minute_Night_Shift);
...
// if the file opened okay, write to it:
        if (myFile) {
            ...
            if (myFile.size() == 0) {
                myFile.println(
                        "hour,minute,second,t0,t1,t2,t3,t4-podlOut,t5-podlIn,buforGrzeje,pompaMiesza,pompaPodlogowa,tNI_r,tNO_r,tUI_r,tUO_r,RPM_NEW,RPM_USED");
            }
